...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def _load_checkpoint_for_eval(path, model):
        def _load(allow_unsafe=False):
        ...
        try:
            checkpoint = _load(allow_unsafe=False)
        except Exception:
            checkpoint = _load(allow_unsafe=True)
    ...
    if has_saved_checkpoint and os.path.isfile(hyp_params.name):
        model = _load_checkpoint_for_eval(hyp_params.name, model)
