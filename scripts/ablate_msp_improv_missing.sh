...
OUTPUT_DIR="${OUTPUT_DIR:-./results/msp_improv_missing_ablation}"
PRETRAIN_CHECKPOINT="${PRETRAIN_CHECKPOINT:-${OUTPUT_DIR}/msp_improv_ablation_pretrain.pt}"
...
for max_missing_prob in ${MAX_MISSING_VALUES}; do
  ...
  finetune_checkpoint="${OUTPUT_DIR}/msp_improv_ablation_${tag}_finetune.pt"
  eval_csv="${OUTPUT_DIR}/msp_improv_ablation_${tag}_eval.csv"
...
for max_missing_prob in max_missing_values:
    ...
    csv_path = output_dir / f"msp_improv_ablation_{tag}_eval.csv"
